...
var processSeasonalCollection = function(year, season, startDate, endDate, aoi, maskConditions, LandTrender, nbrTransform) {
  var collection = LandTrender.getCombinedSRcollection(year.toString(), startDate, endDate, aoi, maskConditions)
    ...
    return collection.median().clip(aoi).select('NBR');
...
function addNBR(image) {
  // Select the NIR (B8) and SWIR (B12) bands from the image
  var nir = image.select('B8');
  var swir = image.select('B12');
  // Calculate NBR using arithmetic operations
  //var nbr = nir.subtract(swir).divide(nir.add(swir)).rename('sentiNBR');
  var nbr = nir.add(swir).divide(nir.subtract(swir)).rename('sentiNBR');
...
function generateSeasonalNBRImages(startYear, endYear) {
 ...
 if(season=='Summer'){
       var range = s2Sr.reduceColumns(ee.Reducer.minMax(), ['system:time_start'])
...
function calculateZScores(predictedSeasonalNBRImageCollections, combinedSeasonalStats) {
    var zScoreSeasonalNBRImageCollections = {};
    //print(predictedSeasonalNBRImageCollections)
    //print(combinedSeasonalStats)
    // Iterate over each season
    Object.keys(predictedSeasonalNBRImageCollections).forEach(function(season) {
        var predictedCollection = predictedSeasonalNBRImageCollections[season];
        var stats = combinedSeasonalStats[season];
        
        var meanImage = stats.mean.select('sentiNBR'); // Assuming the mean image
        var stdDevImage = stats.stdDev.select('sentiNBR_stdDev'); // Assuming the standard deviation image
        
        //print(meanImage)
        //print(stdDevImage)

        // Calculate the Z-score for each image in the collection
        var zScoreCollection = predictedCollection.map(function(image) {
            // Ensure you are working with the PredictedNBR band
            var predictedNBR = image.select('sentiNBR');
            //print(predictedNBR)
            // Calculate the Z-score
            var zScore = predictedNBR.subtract(meanImage).divide(stdDevImage).rename('ZScore');
...
var zScoreSeasonalNBRImageCollections = calculateZScores(predictedSeasonalNBRImageCollections, combinedSeasonalStats);

print(zScoreSeasonalNBRImageCollections)


function exportZScoreImages(year, zScoreSeasonalNBRImageCollections) {
  var yearIndex = year - 1990; // Adjust based on the start year of your collection
  ...
  seasons.forEach(function(season) {
    ...
    //Map.addLayer(image,ZVis, year+"_map of_"+season)
    

    Export.image.toDrive({
      image: image,
      description: year+'North_Coast_'+season+'_clipped',
      folder: 'North_Coast_'+ year,
      scale: 30,
      region: aoi,
      crs:'EPSG:3577',
      fileFormat: 'GeoTIFF',
...
// Example usage:
exportZScoreImages(2016, zScoreSeasonalNBRImageCollections);
